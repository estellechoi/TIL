...
`console.log(tmp)`의 결과는 왜 `undefined` 일까요? 먼저, 에러가 나지 않고 `undefined`가 출력되는 이유는 변수 `tmp`가 호이스팅 되어 참조할 수 있는 상태이기 때문입니다. 한편, 아직 `var tmp = 'World'` 코드가 실행된 것은 아니므로 값은 할당되어 있지 않습니다.

<br>

함수 선언식과 달리, 함수 표현식은 호이스팅 되지 않습니다.
