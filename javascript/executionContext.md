...
실행 컨텍스트들을 담는 LIFO(Last In First Out) 스택 자료구조 입니다. 이 스택은 기본으로 전역 컨텍스트를 담고 있습니다.

![Execution Context Stack](./../img/executionContextStack.png)

<br>

JS 엔진에서 전역 컨텍스트의 코드를 실행시키는 중 함수 호출이 발생하면, 스택의 가장 위에 호출된 함수의 실행 컨텍스트가 담깁니다. 새로운 컨텍스트가 쌓였으므로, JS 엔진은 전역 컨텍스트의 실행을 멈추고 함수 컨텍스트의 작업을 시작합니다.

![Execution Context Stack](./../img/executionContextStack2.png)

<br>

이런 식으로 함수가 호출될 때마다 새로운 실행 컨택스트가 생성되고 기존의 컨텍스트 위에 쌓입니다.

![Execution Context Stack](./../img/executionContextStack3.png)

<br>

함수 컨텍스트의 모든 코드가 실행되면 실행이 완료된 컨텍스트를 걷어내고 정지되었던 컨텍스트들이 차례로 작업을 완료합니다. 모든 함수 컨텍스트의 작업이 완료된 후에 전역 컨택스트가 다시 실행됩니다.
