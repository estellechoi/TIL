...
`new Point(1, 2)`가 실행되면, V8 엔진은 클래스 <b>C0</b>을 생성합니다. 이 때, `Point` 객체 내에 정의된 속성들이 없으므로 클래스 <b>C0</b>은 완전히 비어있는 상태입니다.

`Point` 함수 내의 `this.x = x` 코드가 실행되면 속성 `x`가 `Point` 객체에 추가됩니다. 이때 V8 엔진은 두 번째 클래스인 <b>C1</b>을 생성하고, 클래스 <b>C0</b>은 클래스 <b>C1</b>으로 전환됩니다.(Class Transition) 클래스 <b>C1</b>은 속성 `x`의 메모리 내의 위치를 나타냅니다.
