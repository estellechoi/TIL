...
위 단계를 통해 앱이 생성되면 아래와 같이 앱의 상세정보 페이지로 자동 이동됩니다. 이 페이지는 `나의 앱` 페이지에서 지정한 이름의 앱 항목을 클릭하여 접근할 수 있습니다. 사이드 바의 `앱 정보` 메뉴로 이동하여 번들 ID에 위 단계에서 선택한 고유 앱 ID가 들어있는지 확인하세요.
