...
3. CSS 픽셀, DPPX(Device Pixel Ratio), 4픽셀 베이스라인 그리드(4px Baseline Grid)
4. 베이스라인 그리드 적용 범위: 박스모델(Box Model), 타이포그래피 - Escape 베이스라인 그리드
5. 모듈식 스케일(Modular Scales)
6. 단위: `px`, `rem`, `em`
...
[픽셀](https://en.wikipedia.org/wiki/Pixel)(`px`)은 디지털 디바이스 화면에서 가장 기본이 되는 단위입니다. 화면을 쪼갤 수 있는 가장 작은 단위를 픽셀, 우리 말로는 화소라고 합니다. `mm`나 `inch`와 같이 고정된 물리적인 값을 나타내는 단위가 아니기 때문에 동일한 크기의 화면이라도 픽셀 수는 다를 수 있습니다. 따라서 `1px`의 실제 물리적 크기는 디바이스마다 제각각입니다.
...
### 3-3. 4픽셀 베이스라인 그리드(4px Baseline Grid)
