...
4. 폰트 사이즈(Font Size): 웹접근성, `rem`, Type Scales
...
`px`이 아닌 `rem` 단위를 사용합니다. `1rem`은 `<html>` 태그의 폰트 사이즈입니다. 따라서 기본 사이즈를 `16px`로 지정하려면, `<html>` 태그의 폰트 사이즈를 `16px`로 지정한 후, 다른 모든 폰트 사이즈는 `rem` 단위를 사용하여 지정합니다.
...
텍스트의 줄높이 역시 `line-height: 1.5`와 같이 일괄로 적용한다면 쉽겠지만, 폰트 사이즈별로 서로 다른 배수의 줄높이를 지정하는 것이 좋습니다. 4pt Baseline Grid 레이아웃을 사용한다면 줄높이가 `4`의 배수여야 안정감이 있기 때문입니다.
