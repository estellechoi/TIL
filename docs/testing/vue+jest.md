...
그럼 `package.json` 파일에 아래와 같이 필요한 스크립트와 디펜던시들이 Preset 됩니다. ([TypeScript](https://www.typescriptlang.org/), Babel, Jest 관련 항목이 아닌 것은 생략)
...
- [`@types/jest`](https://www.npmjs.com/package/@types/jest): `.ts` 파일에서 테스트 작성시 `jest`를 사용하기 위해 필요한 Type 정보를 제공
...
- [`@vue/cli-plugin-typescript`](https://cli.vuejs.org/core-plugins/typescript.html): TypeScript + [`ts-loader`](https://github.com/TypeStrong/ts-loader) + [`https://github.com/TypeStrong/fork-ts-checker-webpack-plugin`](https://github.com/TypeStrong/fork-ts-checker-webpack-plugin)

- [`@vue/cli-plugin-unit-jest`](): 4.x 이상 버전에서 [`/presets/typescript-and-babel`](https://github.com/vuejs/vue-cli/tree/dev/packages/%40vue/cli-plugin-unit-jest/presets/typescript-and-babel) Preset 제공. 이전 버전을 사용한다면 Babel의 TypeScript 플러그인 [`@babel/preset-typescript`](https://babeljs.io/docs/en/babel-preset-typescript) 추가 설치 필요. ([Jest에서 Babel을 통해 TypeScript를 지원](https://jestjs.io/docs/getting-started#using-typescript)하기 때문)

- [`@vue/test-utils`](https://github.com/vuejs/vue-test-utils): Vue 컴포넌트에 대한 단위 테스트를 간편하게 해주는 유틸 함수들을 제공

- [`vue-jest`](https://github.com/vuejs/vue-jest): Vue [SFC](https://vuejs.org/v2/guide/single-file-components.html) 파일(`.vue`)내 `<template />`,  `<script />`, `<style />` 구문들을 Jest가 동작할 수 있는 JavaScript 파일로 컴파일

<br>

#### 기존 프로젝트에 추가하기

기존 Vue 프로젝트에 VTU를 추가하는 경우라면, 아래와 같이 `@vue/cli` 명령어를 사용할 수 있습니다. [이 명령어](https://cli.vuejs.org/core-plugins/unit-jest.html#installing-in-an-already-created-project)는 `vue-cli-service test:unit` 커맨드를 추가하고, `@vue/cli-plugin-unit-jest`와 `vue-jest`를 설치합니다.
...
## 2. Jest 설정하기: `jest.config.js`, 변환이 필요한 써드파티 모듈 알려주기, 코드 커버리지
...
[`@vue/cli-plugin-unit-jest/presets/typescript-and-babel`](https://github.com/vuejs/vue-cli/blob/dev/packages/%40vue/cli-plugin-unit-jest/presets/typescript-and-babel/jest-preset.js)을 확인해보면 다음 3 개 Preset 합으로 되어있습니다.
...
- [`presets/typescript/jest-preset`](https://github.com/vuejs/vue-cli/blob/dev/packages/%40vue/cli-plugin-unit-jest/presets/typescript/jest-preset.js): `.ts`/`.tsx` 파일을 `ts-jest`로 컴파일 하도록 설정
...
기본적으로 Jest는 `/node_modules/`에 설치된 디펜던시 모듈들을 컴파일하지 않는데요, 이는 `jest.config.js` 파일의 `transformIgnorePatterns` 옵션 디폴트 값에 `/node_modules/` 경로가 포함되어있기 때문입니다.
...
Jest는 브라우저가 아닌 Node.js에서 Run 됩니다. `node` 8.x 이상부터 거의 대부분의 모던 ECMAScript 기능들을 지원하고있기 때문에, JavaScript 라이브러리 모듈을 테스트 전에 변환시켜야하는 경우가 드물기 때문입니다. 다만 아래의 예외 사항에 해당하는 라이브러리가 있다면 Jest가 테스트하기 전 해당 라이브러리 모듈을 변환할 수 있도록 알려주어야합니다!
...
만약 해당하는 라이브러리가 있다면 Jest가 테스트 전에 적절하게 변환할 수 있도록 다음과 같이 `transformIgnorePatterns` 옵션을 수정합니다. 자세한 설명은 [`@vue/cli-plugin-unit-jest` 공식문서](https://cli.vuejs.org/core-plugins/unit-jest.html#transform-dependencies-from-node-modules)를 확인해보세요.
